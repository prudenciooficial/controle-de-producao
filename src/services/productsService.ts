...
export const fetchProducts = async (): Promise<Product[]> => {
  ...
  return data.map(product => ({
    ...
    productionPredictionFactor: product.production_prediction_factor || 3.5, // Default to 3.5 if not set
...
export const createProduct = async (
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      fecula_conversion_factor: product.feculaConversionFactor || 25, // Default to 25 if not set
      production_prediction_factor: product.productionPredictionFactor || 3.5 // Default to 3.5 if not set
    })
    .select()
    .single();
  ...
  return {
    ...
    productionPredictionFactor: data.production_prediction_factor || 3.5,
